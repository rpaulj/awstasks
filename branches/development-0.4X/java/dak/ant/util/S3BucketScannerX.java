...
public class S3BucketScannerX 
       { // INSTANCE VARIABLES
         ...
         private ConcurrentSkipListSet<S3File> included = new ConcurrentSkipListSet<S3File>();
         ...
         /** Initialises the internal S3 service.
           ...
           */
         public S3BucketScannerX(S3Service service) 
                { // ... validate
         ...
         /** Sets the list of explicitly selected objects.
           * 
           */
         public void setSelected(List<S3File> selected) 
                { String[] keys  = new String[selected.size()];
                  int      index = 0;

                  for (S3File item : selected)
         ...
         public S3File[] getIncludedObjects() 
                { return included.toArray(new S3File[0]);
                }
         ...
         public synchronized void scan() 
                { try 
                     { // ... validate
                       ...
                       if (includes == null)
                          includes = (selected.length == 0) ? new String[] { SelectorUtils.DEEP_TREE_MATCH } : new String[0];
                       ...
                       included = new ConcurrentSkipListSet<S3File>();
                       ...
                       for (S3Object object: list) 
                           { String  key      = object.getKey();
                             ...
                             if (selected || (included && !excluded))
                                this.included.add(new S3File(object));
                           }
                     } 
